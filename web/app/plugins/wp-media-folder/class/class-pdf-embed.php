...
class WpmfPdfEmbed
{
    ...
    public function loadPdfEmbedHtml()
    {
        ...
        $html = do_shortcode('[wpmfpdf is_divi="1" url="'. $url .'" embed="'. $_REQUEST['embed'] .'" target="'. $_REQUEST['target'] .'"]');
    ...
    public function wpmfPdf($attrs)
    {
        ...
        if (!empty($attrs['embed']) && $attrs['embed'] !== 'off') {
            ...
            $return = '<a class="wpmf-pdfemb-viewer" href="'.esc_url($url).'">'.esc_html($title).'</a>';
        } else {
            $return = '<a href="'.esc_url($url).'" target="'.esc_attr($attrs['target']).'">'.esc_html($title).'</a>';
        }
