...
class WpmfHelper
{
    ...
    public static function getCountFiles($term_id)
    {
        ...
        if (defined('ICL_SITEPRESS_VERSION') && ICL_SITEPRESS_VERSION) {
            ...
            $count = $wpdb->get_var($wpdb->prepare('SELECT COUNT(ID) FROM ' . $wpdb->posts . ' as p, ' . $wpdb->term_relationships . ' as tr, ' . $wpdb->term_taxonomy . ' as tt, ' . $wpdb->terms . ' as t WHERE p.ID = tr.object_id AND post_type = "attachment" AND (post_status = "publish" OR post_status = "inherit") AND tt.term_taxonomy_id = tr.term_taxonomy_id AND tt.term_id=t.term_id AND t.term_id = %d', (int)$term_id));
        }
     ...
     */
    public static function attachmentUrlToPostid($url, $ext = '')
    {
        global $wpdb;
